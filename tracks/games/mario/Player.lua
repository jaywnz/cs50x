...
function Player:checkLeftCollision()
    if self.dx < 0 then
        ...
        -- Check for flagpole
        if self.map:flagpoleHit(self.map:tileAt(self.x - 1, self.y)) or self.map:flagpoleHit(self.map:tileAt(self.x - 1, self.y + self.height - 1)) then
            gameState = 'victory'
        end
    end
end
...
function Player:checkRightCollision()
    if self.dx > 0 then
        ...
        -- Check for flagpole
        if self.map:flagpoleHit(self.map:tileAt(self.x + self.width, self.y)) or self.map:flagpoleHit(self.map:tileAt(self.x + self.width, self.y + self.height - 1)) then
            gameState = 'victory'
        end
    end
end
